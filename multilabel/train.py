...
from dataset import load_multilabel, MultiLabelDataset, MultiLabelCollator
...
def train(config_path: Path):
    ...
    data = data.train_test_split(
        test_size=config.eval_split_fraction, shuffle=True, seed=config.random_state
    )
        ...
        for epoch in range(config.epochs):
            for batch_i, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(model):
                    ...
                    accelerator.log(
                        {"train_loss": loss_value.item()}, step=current_step + 1
                    )
                ...
                if (
                    current_step + 1
                ) % config.save_steps == 0 or current_step + 1 == total_steps:
                    accelerator.wait_for_everyone()
                    accelerator.save_model(
                        model, config.save_path / f"step_{current_step + 1}"
                    )
